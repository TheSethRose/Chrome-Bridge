...
2. Inspect narrowly first. Use `capabilities` before uncertain page types. Prefer direct semantic targets such as `click --role=button --name=Save`; zero matches return nearby candidates and multiple matches require `--exact`, `--within`, or zero-based `--nth`. Use `locate` when the target is still unclear, and prefer filtered or compact `snapshot`, bounded `extract`, `visible-text --selector`, `dom --selector`, or `eval --value-only` over returning the entire DOM, network log, or storage database.
